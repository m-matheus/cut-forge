...
def test_premiere_export_roundtrip(run_project, monkeypatch):
    ...
    # The title-card still declares its pixel size so Premiere links it (no "Media offline").
    xml_text = out.read_text(encoding="utf-8")
    assert "<samplecharacteristics>" in xml_text
    assert "1920" in xml_text and "1080" in xml_text


def test_premiere_export_with_caption_overlay(run_project, monkeypatch):
    project, wav_track = run_project
    monkeypatch.setattr(type(project), "track_path",
                        property(lambda self: wav_track))
    alignment = {
    ...
    # Fake the ffmpeg render so the test is fast and ffmpeg-independent: just drop a
    # placeholder .mov at the expected path and return it.
    from cutforge.services import caption_render_service

    def fake_render(proj, **kwargs):
        proj.captions_overlay_path.parent.mkdir(parents=True, exist_ok=True)
        proj.captions_overlay_path.write_bytes(b"\x00" * 16)
        return proj.captions_overlay_path

    monkeypatch.setattr(caption_render_service, "render_caption_overlay", fake_render)

    from cutforge.services import premiere_service
    out = premiere_service.build_project(project, captions_overlay=True)

    back = otio.adapters.read_from_file(str(out), adapter_name="fcp_xml")
    video_tracks = [t for t in back.tracks if t.kind == otio.schema.TrackKind.Video]
    assert len(video_tracks) == 3  # V1 footage + V2 title + V3 caption overlay
    clip_names = {c.name for t in video_tracks for c in t}
    assert "captions" in clip_names
